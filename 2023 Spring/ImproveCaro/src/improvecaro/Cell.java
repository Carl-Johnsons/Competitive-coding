...
public final class Cell extends JLabel {
    ...
    private void cardClicked() {
        parent.setNumberOfCellUnOccupied(parent.getNumberOfCellUnOccupied() - 1);
    ...
    private void handleRPG() {
        ...
        if (!check && checkTieCondition()) {
            parent.getPnlGameNotfication().setTie();
            return;
        }
...
//        System.out.println("Left Up Right Down :" + lurd);
    ...
    private boolean checkTieCondition() {
        return parent.getNumberOfCellUnOccupied() == 0;
    }
    ...
    private boolean checkWinCondition() {
        // ==== Dieu kien de thang va ve luon duong chien thang ====
        int win_condition = (parent.isRPG() || (parent.isNormal() && parent.getPnlNormalMode().getMapType() == NormalCaro.FIVE_FIVE) ? 4 : 2);
    ...
    public void drawWinningLine() {
        ...
        if (parent.isRPG()) {
            ...
            if (parent.getTurn() == ImproveCaro.OCell) {
                parent.getBlueInfo().reducedHp();
            } else {
                parent.getRedInfo().reducedHp();
            }

            if (!(parent.getRedInfo().getHP() > 0
                    && parent.getBlueInfo().getHP() > 0)) {
